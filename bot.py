...
def ensure_meta(owner: str, folder: str):
    ...
    base = {"display_name": folder, "startup_file": "", "owner": owner, "banned": False, "web_port": None, "is_web_server": False}
...
def read_meta(owner: str, folder: str):
    ...
    try:
        ...
        return {"display_name": folder, "startup_file": "", "owner": owner, "banned": False, "web_port": None, "is_web_server": False}
...
def is_api_file(file_path: str) -> bool:
    """اكتشاف إذا كان الملف Flask/FastAPI"""
    try:
        with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
            content = f.read(3000).lower()
            return any(x in content for x in [
                "flask(", "flask.", "app = flask", "app.run(",
                "@app.route", "fastapi(", "app = fastapi",
...
def start_flask_server(owner: str, folder: str, startup_file: str, port: int):
    """تشغيل ملف Flask API كخادم ويب على منفذ محدد"""
    server_dir = get_server_dir(owner, folder)
    log_path = os.path.join(server_dir, "server.log")
    log_file = open(log_path, "a", encoding="utf-8", errors="ignore")
    
    file_path = os.path.join(server_dir, startup_file)
    
    # إنشاء ملف مؤقت مع تعديل المنفذ
    with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
        original_content = f.read()
    
    # تعديل المنفذ في الكود
    new_content = re.sub(r'port\s*=\s*\d+', f'port={port}', original_content)
    new_content = re.sub(r'PORT\s*=\s*\d+', f'PORT={port}', new_content)
    
    # إضافة سطر لتشغيله على 0.0.0.0 إذا لم يكن موجوداً
    if 'host=' not in new_content.lower() and 'app.run(' in new_content:
        new_content = new_content.replace('app.run(', f'app.run(host="0.0.0.0", ')
    
    temp_file = os.path.join(server_dir, f".temp_{startup_file}")
    ...
    # تنظيف الملف المؤقت بعد 5 ثواني
    def cleanup():
        time.sleep(5)
        try:
...
def background_start(key: str, owner: str, folder: str, startup_file: str):
    try:
        ...
        full_startup = os.path.join(get_server_dir(owner, folder), startup_file)
        
        # اكتشاف إذا كان فلاسك API
        if is_api_file(full_startup):
            free_port = get_free_port()
            log_append(key, f"[SYSTEM] 🔗 Flask/FastAPI detected → starting on port {free_port}\n")
            
            proc, logf = start_flask_server(owner, folder, startup_file, free_port)
            running_procs[key] = (proc, logf)
            
            # حفظ المنفذ في meta
            meta = read_meta(owner, folder)
            meta["web_port"] = free_port
            meta["is_web_server"] = True
            write_meta(owner, folder, meta)
            
            local_ip = get_ip()
            log_append(key, f"[SYSTEM] ✅ API Running at: http://{local_ip}:{free_port}\n")
            log_append(key, f"[SYSTEM] 🌍 Public URL: http://{local_ip}:{free_port}\n")
        else:
            # بوت عادي
            log_append(key, "[SYSTEM] Starting as Bot...\n")
    ...
    for owner in sorted(os.listdir(USERS_ROOT)):
        ...
        for folder in sorted(os.listdir(root)):
            ...
            servers.append({
                ...
                "is_web_server": meta.get("is_web_server", False)
            })
    return servers
...
def server_stats(key):
    ...
    return jsonify({
        ...
        "web_port": meta.get("web_port"),
        "is_web_server": meta.get("is_web_server", False),
        "web_url": f"http://{get_ip()}:{meta.get('web_port')}" if meta.get('web_port') else None
    })
...
def server_action(key, act):
    ...
    if act in ("stop", "restart"):
        # مسح المنفذ من meta عند الإيقاف
        if act == "stop":
            meta["web_port"] = None
            meta["is_web_server"] = False
            write_meta(owner, folder, meta)
        stop_proc(key)
        set_state(key, "Offline")
...
def files_list(key):
    ...
    if os.path.isdir(base):
        for name in sorted(os.listdir(base), key=lambda x: (not os.path.isdir(os.path.join(base, x)), x.lower())):
            if rel == "" and name in ("meta.json", "server.log", ".installed"):
                continue
...
# ---------------------------
# Extra API: Get web URL for a server
# ---------------------------
@app.route("/server/weburl/<path:key>")
@login_required
def server_web_url(key):
    if not can_access_key(key):
        return jsonify({"success": False, "message": "Forbidden"}), 403
    ...
    if meta.get("is_web_server") and meta.get("web_port"):
        return jsonify({
            "success": True,
            "url": f"http://{get_ip()}:{meta['web_port']}",
            "port": meta['web_port']
        })
    return jsonify({"success": False, "message": "Not a web server or not running"})
